	...
	public class MathTypeConverter : ExpandableObjectConverter
	{
		...
		internal static T[] ConvertToValues<T>(ITypeDescriptorContext context, CultureInfo culture, object value, int arrayCount, params string[] expectedParams)
		{
			...
			for (int i = 0; i < localArray.Length; i++)
			{
				...
				{
					throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, "Invalid string", new object[] { string.Join(culture.TextInfo.ListSeparator, expectedParams) }), exception);
				}
			}
			if (localArray.Length != arrayCount)
			{
				throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, "Invalid string", new object[] { string.Join(culture.TextInfo.ListSeparator, expectedParams) }));
			}
			return localArray;
		}
